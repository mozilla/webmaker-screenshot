...
describe("app", function() {
  ...
  describe("thumbnail endpoints", function() {
    ...
    describe("GET /:viewport/:thumbnail/:key", function() {
    ...
    describe("POST /:viewport/:key", function() {
      it("returns 400 when make doesn't exist", function(done) {
        verifyIsHtml = function(url, cb) {
          cb(null, false);
        };

        request(app)
          .post('/desktop/large/t.makes.org/blah')
          .expect(400)
          .expect({error: 'URL is not HTML'})
          .end(done);
      });

      it("returns 200 on screenshot success", function(done) {
        verifyIsHtml = function(url, cb) {
          cb(null, true);
        };
